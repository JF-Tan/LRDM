...
from lrdm.datasets.get_dataset import dataset
...
def calc_psnr(sr, hr, scale=1, rgb_range=255., benchmark=True):
...
def calc_ssim(img1, img2, scale=1, benchmark=True):
    '''calculate SSIM
...
class ResidualDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.enable_res_low_rank = enable_res_low_rank
    ...
    def p_losses(self, imgs, t, noise=None):
        ...
        ######################### LOW-RANK ##############################################
        if self.enable_res_low_rank:
            b,c,h,w = x_res.shape
            I_res_flat = x_res.view(-1,h,w)  
            U, S, Vh = torch.linalg.svd(I_res_flat) 

            if isinstance(self.rank_func, int):
                k = self.rank_func
            else:
                if self.rank_func == 'lin_decrease':
                    normed_t = t / self.num_timesteps
                    rank_lin_decrease = torch.ceil((1-normed_t) * self.image_size)
                    k = rank_lin_decrease
                elif self.rank_func == 'lin_increase':
                    normed_t = t / self.num_timesteps
                    rank_lin_increase = torch.ceil(normed_t * self.image_size)
                    k = rank_lin_increase
                elif self.rank_func == 'poly_increase':
                    normed_t = t / self.num_timesteps
                    rank_poly_increase = torch.ceil(1-envelope_fn2(normed_t,3) *  self.image_size)
                    k = rank_poly_increase
                elif self.rank_func == 'poly_decrease':
                    normed_t = t / self.num_timesteps
                    rank_poly_decrease = torch.ceil(envelope_fn2(normed_t,3) * self.image_size)
                    k = rank_poly_decrease
                
                #--------------- poly increase -----------------------
                elif self.rank_func == 'poly_1_increase':
                    normed_t = t / self.num_timesteps
                    rank_poly_increase = torch.ceil(1-envelope_fn2(normed_t,1) *  self.image_size)
                    k = rank_poly_increase
                elif self.rank_func == 'poly_2_increase':
                    normed_t = t / self.num_timesteps
                    rank_poly_increase = torch.ceil(1-envelope_fn2(normed_t,2) *  self.image_size)
                    k = rank_poly_increase
                elif self.rank_func == 'poly_3_increase':
                    normed_t = t / self.num_timesteps
                    rank_poly_increase = torch.ceil(1-envelope_fn2(normed_t,3) *  self.image_size)
                    k = rank_poly_increase
                elif self.rank_func == 'poly_4_increase':
                    normed_t = t / self.num_timesteps
                    rank_poly_increase = torch.ceil(1-envelope_fn2(normed_t,4) *  self.image_size)
                    k = rank_poly_increase
                elif self.rank_func == 'poly_5_increase':
                    normed_t = t / self.num_timesteps
                    rank_poly_increase = torch.ceil(1-envelope_fn2(normed_t,5) *  self.image_size)
                    k = rank_poly_increase
                
                #--------------- poly decrease -----------------------
                elif self.rank_func == 'poly_1_decrease':
                    normed_t = t / self.num_timesteps
                    rank_poly_decrease = torch.ceil(envelope_fn2(normed_t,1) *  self.image_size)
                    k = rank_poly_decrease
                elif self.rank_func == 'poly_2_decrease':
                    normed_t = t / self.num_timesteps
                    rank_poly_decrease = torch.ceil(envelope_fn2(normed_t,2) *  self.image_size)
                    k = rank_poly_decrease
                elif self.rank_func == 'poly_3_decrease':
                    normed_t = t / self.num_timesteps
                    rank_poly_decrease = torch.ceil(envelope_fn2(normed_t,3) *  self.image_size)
                    k = rank_poly_decrease
                elif self.rank_func == 'poly_4_decrease':
                    normed_t = t / self.num_timesteps
                    rank_poly_decrease = torch.ceil(envelope_fn2(normed_t,4) *  self.image_size)
                    k = rank_poly_decrease
                elif self.rank_func == 'poly_5_decrease':
                    normed_t = t / self.num_timesteps
                    rank_poly_decrease = torch.ceil(envelope_fn2(normed_t,5) *  self.image_size)
                    k = rank_poly_decrease
                k = k.long()

            I_res_lowrank = U[:, :, :k] @ torch.diag_embed(S[:, :k]) @ Vh[:, :k, :]
            I_res_lowrank = I_res_lowrank.view_as(x_res)
            x_res = I_res_lowrank
    ...
    def __init__(
        ...
        *,
        ...
        if optimizer_cls == 'Adam':
            ...
            self.opt = RAdam(diffusion_model.parameters(),
                            lr=train_lr, betas=adam_betas)
        ...
        self.device = device
        # timing / FLOPs statistics
        self.total_time = 0.0
        self.flops = None
        self.params = None

        # try to compute FLOPs once (best-effort, optional dependency: thop)
        if _has_thop:
            try:
                self.model.eval()
                with torch.no_grad():
                    # create a representative dummy input matching training-time shape
                    if self.condition:
                        # for conditional models the dataset yields lists; we pass a tuple containing a list
                        dummy = torch.randn(1, self.image_size and self.model.channels or self.model.channels, self.image_size, self.image_size, device=self.device)
                        # many dataset configurations use [gt, input] form
                        inputs = ([dummy, dummy],)
                    else:
                        dummy = torch.randn(1, self.model.channels, self.image_size, self.image_size, device=self.device)
                        inputs = (dummy,)
                    flops, params = profile(self.model, inputs=inputs, verbose=False)
                    self.flops = flops
                    self.params = params
            except Exception as e:
                print("FLOPs profiling skipped (thop failed):", e)
    ...
    def train(self):
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                # measure step time (includes forward/backward/opt step)
    ...
    def test(self, sample=False, last=True, FID=False):
        ...
        if self.condition:
            ...
            for items in loader:
                ...
                sample_results = all_images
                gt = show_x_input_sample[0]
                # all_images = torch.cat([show_x_input_sample[1], all_images, gt], dim=0)

                mat_s = sample_results * 255
                mat_gt = gt * 255
                psnr = calc_psnr(mat_s, mat_gt)
                ssim = calc_ssim(mat_s, mat_gt)

                print(f"test-{file_name} PSNR: {psnr:.4f} SSIM: {ssim:.4f}")
                ...
                utils.save_image(all_images, str(
                    self.results_folder / file_name), nrow=nrow)
                # utils.save_image(gt, str(
                #     self.results_folder / f'gt-{file_name}'), nrow=1)
        ...
        # 将结果写入 txt（追加模式）
        summary_path = self.results_folder / 'test_summary.txt'
        try:
            with open(summary_path, 'w') as fw:
                fw.write(f"Average PSNR: {avg_psnr}\n")
                fw.write(f"Average SSIM: {avg_ssim}\n")
                fw.write("test end\n")
        except Exception as e:
            print(f"Failed to write test summary to {summary_path}: {e}")
